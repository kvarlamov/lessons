...
/*
 * Рефакторинг :
 * Создал отдельный метод, который в зависимости от типа будет выполнять мок
 * Т.о. вместо цепочки вызовов -- явно определил, какой метод выполняется
 * Правда появилась другая проблема -- ипсользование if.
 * Её нужно решать отдельно, например через полиморфизм, но в рамках данного задания ограничился if (switch-case)
